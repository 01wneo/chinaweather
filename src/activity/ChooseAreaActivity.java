...
public class ChooseAreaActivity extends Activity {
	...
	private void queryCities() {
		cityList = chinaWeatherDB.loadCities(selectedProvince.getId());
		if (cityList.size() > 0) {
			dataList.clear();
		...
		HttpUtil.sendHttpRequest(address, new HttpCallbackListener() {
			...
			public void onFinish(String response) {
				...
				if ("province".equals(type)) {
					result = Utility.handleProvinceResponse(chinaWeatherDB,
							response);
				} else if ("city".equals(type)) {
					result = Utility.handlerCitiesReaponse(chinaWeatherDB,
							response, selectedProvince.getId());
				} else if ("county".equals(type)) {
					result = Utility.handlerCountiesReaponse(chinaWeatherDB,
			...
			public void onError(Exception e) {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						// TODO Auto-generated method stub
						closeProgressDialog();
						Toast.makeText(ChooseAreaActivity.this, "网络不行啊！",
	...
	private void showProgressDialog() {
		if (progressDialog == null) {
			...
			progressDialog.setMessage("即将显示");
